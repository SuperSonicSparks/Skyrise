...
#pragma config(Sensor, in8,    Gryo,           sensorGyro)
...
#pragma config(Sensor, dgtl6,  SkyriseSol,     sensorDigitalOut)
...
#pragma config(Sensor, I2C_5,  leftFrontI2C,   sensorQuadEncoderOnI2CPort,    , AutoAssign)
#pragma config(Sensor, I2C_6,  rightFrontI2C,  sensorQuadEncoderOnI2CPort,    , AutoAssign)
#pragma config(Motor,  port2,           leftSideYCable, tmotorVex393_MC29, openLoop, reversed)
#pragma config(Motor,  port3,           leftSideLift,  tmotorVex393_MC29, openLoop, encoderPort, I2C_1)
#pragma config(Motor,  port4,           backLeft,      tmotorVex393_MC29, openLoop, encoderPort, I2C_4)
#pragma config(Motor,  port5,           frontLeft,     tmotorVex393_MC29, openLoop, encoderPort, I2C_5)
#pragma config(Motor,  port6,           frontRight,    tmotorVex393_MC29, openLoop, encoderPort, I2C_6)
#pragma config(Motor,  port7,           backRight,     tmotorVex393_MC29, openLoop, reversed, encoderPort, I2C_3)
#pragma config(Motor,  port8,           rightSideLift, tmotorVex393_MC29, openLoop, encoderPort, I2C_2)
#pragma config(Motor,  port9,           rightSideYCable, tmotorVex393_MC29, openLoop)
...
int stoppingPoint = DISTANCE_TO_GOAL_MM;
bool shoveCube = true;
...
task usercontrol()
{
	...
	/*	if(SensorValue(leftLiftI2C)<10)
		...
		// lift section
		liftLeft = (-vexRT[Ch3Xmtr2]-vexRT[Ch2Xmtr2])/2;
		liftRight = (-vexRT[Ch3Xmtr2]-vexRT[Ch2Xmtr2])/2;

		motor[leftSideYCable] = liftLeft;
		motor[leftSideLift] = liftLeft;
		motor[rightSideYCable] = liftRight;
		motor[rightSideLift] = liftRight;

		//skyrise
		if(vexRT[Btn5U] == 1)
		{
			SensorValue[SkyriseSol] = 0;
		}
		if(vexRT[Btn5D] == 1)
		{
			SensorValue[SkyriseSol] = 1;
		}
		if(vexRT[Btn6D] == 1)
		{
			SensorValue[SkyriseSol] = 1;
...
void openClaw()
{
	SensorValue[SkyriseSol] = CLAW_OPEN;
}
...
void closeClaw()
{
	SensorValue[SkyriseSol] = CLAW_CLOSE;
}
...
void raiseLift(int stopPosition, int liftPower) {
	...
	while (abs(SensorValue[leftLiftI2C]) < stopPosition) {
		...
		motor[leftSideLift] = -liftPower;
		motor[rightSideYCable] = -liftPower; //rightLiftI2C;
		motor[rightSideLift] = -liftPower;
	}
...
void lowerLift(int liftStopPosition, int liftPowerDown, int liftPowerHold) {
	...
	if (currentPosition > liftStopPosition)
	{
		while (currentPosition > liftStopPosition) {
			...
			motor[leftSideLift] = liftPowerDown;
			motor[rightSideYCable] = liftPowerDown;  // rightLiftI2C;
			motor[rightSideLift] = liftPowerDown;
	...
	motor[leftSideLift] = liftPowerHold;
	motor[rightSideYCable] = liftPowerHold; // rightLiftI2C;
	motor[rightSideLift] = liftPowerHold;
}
...
void rampDownMotorsInReverseWithSonar()
{
	while (currentMotorPower > MOTOR_POWER_MIN
		&& currentPosition < stoppingPoint -50)
	{
		// SensorL > SensorR
		// robot is rotating counterclockwise
		// slow down left motors
...
{
	while (currentPosition < (stoppingPoint - 250))
	{
...
void scoreSkyriseSection(bool shoveCube)
{
	...
	stoppingPoint = DISTANCE_TO_GOAL_MM;

	if (shoveCube)
	{
		stoppingPoint = DISTANCE_TO_GOAL_MM;
		currentMotorPower = 80;
		while ((SensorValue[SonarL]) > stoppingPoint+250)
		{
				motor[backLeft] = currentMotorPower;
				motor[backRight] = currentMotorPower;
				motor[frontLeft] = currentMotorPower;
...
void CubeAuton()
{
	...
	int powerInverter = 1;
	if (AUTON_MODE == AUTON_MODE_CUBE_SCORER_RED)
	{
		powerInverter = -1;
	}
...
void SkyriseAuton()
{
	...
	grabHeight = 85;
	initialCarryHeight = 120;
	maxHeight = 120;
	raiseLiftStartingDistance = 100;
	lowerLiftStartingDistance = 300;
	stoppingPoint = DISTANCE_TO_GOAL_MM + 50;
	scoreHeight = 5;
	shoveCube = true;
	abortAuton = false;
